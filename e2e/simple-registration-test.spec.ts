...
test('Simple registration test', async ({ page }) => {
  ...
  console.warn('Body text:', bodyText?.substring(0, 300));
  ...
  console.warn('Visible elements:', visibleElements);
  ...
  console.warn(`Found: ${emailInputs} email inputs, ${passwordInputs} password inputs, ${buttons} buttons`);

  // Check for loading states
  const loadingText = await page.locator('text=Loading').count();
  console.warn('Loading indicators:', loadingText);
